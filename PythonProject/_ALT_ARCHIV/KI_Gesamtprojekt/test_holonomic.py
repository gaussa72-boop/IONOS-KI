...
def test_HolonomicFunction_addition():
    ...
    r = HolonomicFunction((-x ** 18 + 45 * x ** 14 - 525 * x ** 10 + 1575 * x ** 6 - x ** 3 - 630 * x ** 2) + \
                          (-x ** 15 + 30 * x ** 11 - 195 * x ** 7 + 210 * x ** 3 - 1) * Dx + (
                                      x ** 18 - 45 * x ** 14 + 525 * x ** 10 - \
                                      1575 * x ** 6 + x ** 3 + 630 * x ** 2) * Dx ** 5 + (
                                      x ** 15 - 30 * x ** 11 + 195 * x ** 7 - 210 * x ** 3 + \
                                      1) * Dx ** 6, x)
    assert p + q == r
...
def test_HolonomicFunction_multiplication():
    ...
    r = HolonomicFunction((4 * x ** 7 + 11 * x ** 6 + 16 * x ** 5 + 4 * x ** 4 - 6 * x ** 3 - 7 * x ** 2 - 8 * x - 2) + \
                          ...
                          (8 * x ** 6 + 18 * x ** 5 + 15 * x ** 4 - 3 * x ** 3 - 6 * x ** 2 - 6 * x - 2) * Dx ** 2 + (
                                      8 * x ** 5 + \
                                      10 * x ** 4 + 6 * x ** 3 - 2 * x ** 2 - 4 * x) * Dx ** 3 + (
                                      4 * x ** 5 + 3 * x ** 4 - x ** 2) * Dx ** 4, x)
    assert p * q == r
...
def test_addition_initial_condition():
    ...
    r = HolonomicFunction(
        (-x ** 4 - x ** 3 / 4 - x ** 2 + Rational(1, 4)) + (x ** 3 + x ** 2 / 4 + x * Rational(3, 4) + 1) * Dx + \
        (x * Rational(-3, 2) + Rational(7, 4)) * Dx ** 2 + (x ** 2 - x * Rational(7, 4) + Rational(1, 4)) * Dx ** 3 + (
                    x ** 2 + x / 4 + S.Half) * Dx ** 4, x, 0, [2, 2, -2, 2])
    assert p + q == r
...
def test_multiplication_initial_condition():
    ...
    r = HolonomicFunction((x ** 8 - 37 * x ** 7 / 27 - 10 * x ** 6 / 27 - 164 * x ** 5 / 9 - 184 * x ** 4 / 9 + \
                           160 * x ** 3 / 27 + 404 * x ** 2 / 9 + 8 * x + Rational(40, 3)) + (
                                      6 * x ** 7 - 128 * x ** 6 / 9 - 98 * x ** 5 / 9 - 28 * x ** 4 / 9 + \
                                      8 * x ** 3 / 9 + 28 * x ** 2 + x * Rational(40, 9) - 40) * Dx + (
                                      3 * x ** 6 - 82 * x ** 5 / 9 + 76 * x ** 4 / 9 + 4 * x ** 3 / 3 + \
                                      220 * x ** 2 / 9 - x * Rational(80, 3)) * Dx ** 2 + (
                                      -2 * x ** 6 + 128 * x ** 5 / 27 - 2 * x ** 4 / 3 - 80 * x ** 2 / 9 + Rational(200,
                                                                                                                    9)) * Dx ** 3 + \
                          (3 * x ** 5 - 64 * x ** 4 / 9 - 28 * x ** 3 / 9 + 6 * x ** 2 - x * Rational(20, 9) - Rational(
                              20, 3)) * Dx ** 4 + (-4 * x ** 3 + 64 * x ** 2 / 9 + \
                                                   x * Rational(8, 3)) * Dx ** 5 + (
                                      x ** 4 - 64 * x ** 3 / 27 - 4 * x ** 2 / 3 + Rational(20, 9)) * Dx ** 6, x, 0,
                          [3, 3, 3, -3, -12, -24])
    assert p * q == r
...
def test_HolonomicFunction_composition():
    ...
    r = HolonomicFunction((216 * x ** 9 + 324 * x ** 8 + 180 * x ** 7 + 152 * x ** 6 + 112 * x ** 5 + \
                           36 * x ** 4 + 4 * x ** 3) + (24 * x ** 4 + 16 * x ** 3 + 3 * x ** 2 - 6 * x - 1) * Dx + (
                                      6 * x ** 5 + 5 * x ** 4 + \
                                      x ** 3 + 3 * x ** 2 + x) * Dx ** 2, x)
    assert p == r
    ...
    r = HolonomicFunction((x ** 12 + 6 * x ** 10 + 12 * x ** 9 + 15 * x ** 8 + 48 * x ** 7 + 68 * x ** 6 + \
                           72 * x ** 5 + 111 * x ** 4 + 112 * x ** 3 + 54 * x ** 2 + 12 * x + 1) + (
                                      12 * x ** 8 + 32 * x ** 6 + \
                                      24 * x ** 4 - 4) * Dx + (
                                      x ** 12 + 6 * x ** 10 + 4 * x ** 9 + 15 * x ** 8 + 16 * x ** 7 + 20 * x ** 6 + 24 * x ** 5 + \
...
def test_from_hyper():
    ...
    q = HolonomicFunction((4 * x) + (5 * x ** 2 - 8) * Dx + (x ** 3 - 4 * x) * Dx ** 2, x, 1,
...
def test_to_Sequence():
    ...
    q = [(HolonomicSequence(
        2 * n + (3 * n ** 4 - 6 * n ** 3 - 3 * n ** 2 + 6 * n) * Sn + (n ** 3 + 3 * n ** 2 + 2 * n) * Sn ** 2), 0, 1)]
    assert p == q
...
def test_to_Sequence_Initial_Coniditons():
    ...
    q = [
        (HolonomicSequence(n + Sn ** 2 + (n ** 2 + 7 * n + 12) * Sn ** 4, [2, 3, -1, Rational(-1, 2), Rational(1, 12)]),
         1)]
    assert p == q
    ...
    q = [(HolonomicSequence((2 * n ** 2 - 2 * n) + (n ** 3 + 2 * n ** 2 - n - 2) * Sn ** 2,
...
def test_series():
    ...
    assert expr_to_holonomic((2 * x - 3 * x ** 2) ** Rational(1, 3)).series() == (
    ...
    assert expr_to_holonomic(cos(x) ** 2 / x ** 2, y0={-2: [1, 0, -1]}).series(n=10) == (cos(x) ** 2 / x ** 2).series(
        n=10)
    assert expr_to_holonomic(cos(x) ** 2 / x ** 2, x0=1).series(n=10).together() == (cos(x) ** 2 / x ** 2).series(n=10,
                                                                                                                  x0=1).together()
...
def test_expr_to_holonomic():
    ...
    q = HolonomicFunction(
        (-x ** 2 - x / 2 + S.Half) + (x ** 2 - x / 2 - Rational(3, 2)) * Dx + (-x ** 2 + x / 2 + 1) * Dx ** 2 + \
