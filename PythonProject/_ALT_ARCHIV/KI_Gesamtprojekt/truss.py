...
class Truss:
                    ...
                    if node[0] == label:
                        ...
                        if new_label in self._supports:
                            if self._supports[new_label] == 'pinned':
                                if 'R_' + str(label) + '_x' in self._reaction_loads and 'R_' + str(
                                        label) + '_y' in self._reaction_loads:
    ...
    def solve(self):
        ...
        for member in self._members:
            ...
            length = sqrt((self._node_coordinates[start][0] - self._node_coordinates[end][0]) ** 2 + (
    ...
    def draw(self, subs_dict=None):
        ...
        if lim == xmax * 1.1 - xmin * 0.8 + 1:
            sing_plot = plot(1, (x, 1, 1), markers=markers, show=False, annotations=annotations,
                             xlim=(xmin - 0.05 * lim, xmax * 1.1), ylim=(xmin - 0.05 * lim, xmax * 1.1), axis=False,
                             rectangles=rectangles)

        else:
            sing_plot = plot(1, (x, 1, 1), markers=markers, show=False, annotations=annotations,
                             xlim=(ymin - 0.05 * lim, ymax * 1.1), ylim=(ymin - 0.05 * lim, ymax * 1.1), axis=False,
                             rectangles=rectangles)
            ...
            if x2 != x1 and y2 != y1:
                if x2 > x1:
                    member_rectangles.append(
                        {
                            'xy': (x1 - 0.005 * max_diff * cos(pi / 4 + atan((y2 - y1) / (x2 - x1))) / 2,
                    ...
                    member_rectangles.append(
                        {
                            'xy': (x2 - 0.005 * max_diff * cos(pi / 4 + atan((y2 - y1) / (x2 - x1))) / 2,
                ...
                load_annotations.append(
                    {
                        ...
                        'xytext': (
                            x - (max_diff / 100 + abs(xmax - xmin) + abs(ymax - ymin)) * math.cos(
                                pi * load[1] / 180) / 20,
                            y - (max_diff / 100 + abs(xmax - xmin) + abs(ymax - ymin)) * math.sin(
                                pi * load[1] / 180) / 20
                        ),
