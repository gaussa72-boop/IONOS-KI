...
class Beam:
    ...
    * a distributed constant load q2, applied upward, starting from 3*L/4
    ...
    def _solve_hinge_beams(self, *reactions):
        ...
        for load in self.applied_loads:
            if load[1] < l:
                ...
                if load[2] == 0:
                    ...
                    load_1 -= load[0] * SingularityFunction(x, load[3], load[2]) + load[0] * SingularityFunction(x,
                                                                                                                 load[
                                                                                                                     3],
                                                                                                                 0)
            elif load[1] == l:
                ...
                if load[2] == 0:
                    ...
                    load_2 -= load[0] * SingularityFunction(x, load[3] - l, load[2]) + load[0] * SingularityFunction(x,
                                                                                                                     load[
                                                                                                                         3] - l,
                                                                                                                     0)
        ...
        self._hinge_beam_slope = slope_1 * SingularityFunction(x, 0, 0) - slope_1 * SingularityFunction(x, l,
                                                                                                        0) + slope_2 * SingularityFunction(
            x, l, 0)
        self._hinge_beam_deflection = def_1 * SingularityFunction(x, 0, 0) - def_1 * SingularityFunction(x, l,
                                                                                                         0) + def_2 * SingularityFunction(
            x, l, 0)
    ...
    def max_shear_force(self):
        ...
        for i, s in enumerate(singularity):
            if s == 0:
                continue
            try:
                shear_slope = Piecewise((float("nan"), x <= singularity[i - 1]), (self._load.rewrite(Piecewise), x < s),
                                        (float("nan"), True))
            ...
            # solve can't represent Interval solutions.
                ...
                if shear_curve.subs(x, (singularity[i - 1] + s) / 2) == (
                        initial_shear + final_shear) / 2 and initial_shear != final_shear:
        ...
        for i, s in enumerate(singularity):
            ...
            # can't represent Interval solutions.
                ...
                if bending_curve.subs(x, (singularity[i - 1] + s) / 2) == (
                        initial_moment + final_moment) / 2 and initial_moment != final_moment:
    ...
    def deflection(self):
        ...
        if not self._boundary_conditions['deflection'] and not self._boundary_conditions['slope']:
            ...
            return S.One / (E * I) * integrate(-integrate(self.bending_moment(), x), x) + constants[0] * x + constants[
                1]
    ...
    def _draw_load(self, pictorial, length, l):
        ...
        for load in loads:
            ...
            if load[2] == -1:
                ...
                if iln:
                    annotations.append({'text': '', 'xy': (pos, 0), 'xytext': (pos, height - 4 * height),
                                        'arrowprops': {'width': 1.5, 'headlength': 5, 'headwidth': 5,
                                                       'facecolor': 'black'}})
                else:
                    annotations.append({'text': '', 'xy': (pos, height), 'xytext': (pos, height * 4),
                                        'arrowprops': {"width": 1.5, "headlength": 4, "headwidth": 4,
                                                       "facecolor": 'black'}})
            # moment loads
            elif load[2] == -2:
                iln = self._is_load_negative(load[0])
            ...
            if support[1] == "pin":
                ...
                if pos == 0:
                    support_rectangles.append(
                        {'xy': (0, -3 * height), 'width': -length / 20, 'height': 6 * height + height, 'fill': False,
                         'hatch': '/////'})
                else:
                    support_rectangles.append(
                        {'xy': (length, -3 * height), 'width': length / 20, 'height': 6 * height + height,
                         'fill': False, 'hatch': '/////'})
...
class Beam3D(Beam):
    ...
    def shear_stress(self):
        ...
        curve of the Beam object along all three axes.
        """
        return [self.shear_force()[0] / self._area, self.shear_force()[1] / self._area,
    ...
    def solve_for_torsion(self):
        ...
        for i in range(len(pointsList))[1:]:
            ...
            torque_diagram += Piecewise((0, x <= pointsList[i - 1]), (sum_moments, x <= pointsList[i]),
                                        (0, x >= pointsList[i]))
    ...
    def _plot_bending_moment(self, dir, subs=None):
        ...
        return plot(bending_moment[dir_num].subs(subs), (self.variable, 0, length), show=False,
    ...
    def _plot_slope(self, dir, subs=None):
        ...
        return plot(slope[dir_num].subs(subs), (self.variable, 0, length), show=False,
    ...
    def _plot_deflection(self, dir, subs=None):
        ...
        return plot(deflection[dir_num].subs(subs), (self.variable, 0, length), show=False,
    ...
    def _plot_shear_stress(self, dir, subs=None):
        ...
        return plot(shear_stress[dir_num].subs(subs), (self.variable, 0, length), show=False,
