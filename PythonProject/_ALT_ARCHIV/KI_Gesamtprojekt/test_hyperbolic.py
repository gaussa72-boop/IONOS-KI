...
from sympy.functions.elementary.hyperbolic import (acosh, acoth, acsch, asech, asinh, atanh, cosh, coth, csch, sech,
                                                   sinh, tanh)
...
def test_asinh_nseries():
    ...
    assert asinh(2 * I + I * x - x ** 2)._eval_nseries(x, 4, None) == I * pi / 2 + log(2 - sqrt(3)) - \
           sqrt(3) * x / 3 + x ** 2 * (sqrt(3) / 9 - sqrt(3) * I / 3) + x ** 3 * (
                       -sqrt(3) / 18 + 2 * sqrt(3) * I / 9) + O(x ** 4)
...
def test_acosh_nseries():
    ...
    assert acosh(x + 1)._eval_nseries(x, 4, None) == sqrt(2) * sqrt(x) - \
           sqrt(2) * x ** (S(3) / 2) / 12 + 3 * sqrt(2) * x ** (S(5) / 2) / 160 - 5 * sqrt(2) * x ** (
                       S(7) / 2) / 896 + O(x ** 4)
    # Tests concerning points lying on branch cuts
...
def test_asech_nseries():
    ...
    assert asech(x - 1)._eval_nseries(x, 4, None) == I * pi + sqrt(2) * sqrt(x) + \
           5 * sqrt(2) * x ** (S(3) / 2) / 12 + 43 * sqrt(2) * x ** (S(5) / 2) / 160 + 177 * sqrt(2) * x ** (
                       S(7) / 2) / 896 + O(x ** 4)
...
def test_asech_rewrite():
    ...
    assert asech(x).rewrite(atanh) == \
           sqrt(x + 1) * sqrt(1 / (x + 1)) * atanh(sqrt(1 - x ** 2)) + I * pi * (
                       -sqrt(x) * sqrt(1 / x) + 1 - I * sqrt(x ** 2) / (2 * sqrt(-x ** 2)) - I * sqrt(-x) / (
                           2 * sqrt(x)))
...
def test_atanh_rewrite():
    ...
    assert atanh(x).rewrite(asinh) == \
           pi * x / (2 * sqrt(-x ** 2)) - sqrt(-x) * sqrt(1 - x ** 2) * sqrt(1 / (x ** 2 - 1)) * asinh(
        sqrt(1 / (x ** 2 - 1))) / sqrt(x)
...
def test_atanh_nseries():
    ...
    assert atanh(-I * x ** 2 + x - 2)._eval_nseries(x, 4, None) == -I * pi / 2 - log(3) / 2 - x / 3 + \
           x ** 2 * (-S(1) / 4 + I / 2) + x ** 2 * (S(1) / 36 - I / 6) + x ** 3 * (-S(1) / 6 + I / 2) + x ** 3 * (
                       S(1) / 162 - I / 18) + O(x ** 4)
...
def test_acoth_rewrite():
    ...
    assert acoth(x).rewrite(asinh) == \
           x * sqrt(x ** (-2)) * asinh(sqrt(1 / (x ** 2 - 1))) + I * pi * (
                       sqrt((x - 1) / x) * sqrt(x / (x - 1)) - sqrt(x / (x + 1)) * sqrt(1 + 1 / x)) / 2
...
def test_acoth_nseries():
    ...
    assert acoth(-I * x ** 2 - x - S(1) / 2)._eval_nseries(x, 4, None) == I * pi / 2 - log(3) / 2 - \
           4 * x / 3 + x ** 2 * (-S(8) / 9 + 2 * I / 3) - 2 * I * x ** 2 + x ** 3 * (
                       S(104) / 81 - 16 * I / 9) - 8 * x ** 3 / 3 + O(x ** 4)
...
def test_complex():
    ...
    for deep in [True, False]:
        ...
        assert csch(z).expand(complex=True, deep=deep) == cos(b) * sinh(a) / (sin(b) ** 2 \
                                                                              * cosh(a) ** 2 + cos(b) ** 2 * sinh(
                    a) ** 2) - I * sin(b) * cosh(a) / (sin(b) ** 2 \
                                                       * cosh(a) ** 2 + cos(b) ** 2 * sinh(a) ** 2)
        assert sech(z).expand(complex=True, deep=deep) == cos(b) * cosh(a) / (sin(b) ** 2 \
                                                                              * sinh(a) ** 2 + cos(b) ** 2 * cosh(
                    a) ** 2) - I * sin(b) * sinh(a) / (sin(b) ** 2 \
