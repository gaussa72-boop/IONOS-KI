...
public class IonosConnector : MonoBehaviour
{
    public void SendMessageToAI(string message, string mode)
    {
        StartCoroutine(PostRequest(message, mode));
    }

    IEnumerator PostRequest(string message, string mode)
    {
        string json = "{\"message\":\"" + message + "\", \"mode\":\"" + mode + "\"}";

        UnityWebRequest request = new UnityWebRequest("http://127.0.0.1:8000/ionos", "POST");
