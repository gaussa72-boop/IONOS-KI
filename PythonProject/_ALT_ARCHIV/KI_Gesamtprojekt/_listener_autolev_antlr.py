...
def processVariables(self, ctx):
    ...
    # Process variables of the type: Variables x', y''
        ...
        for i in range(ctx.getChildCount()):
            ...
            if self.getValue(ctx.parentCtx.getChild(0)) in ("variable", "specified", "motionvariable",
                                                            "motionvariable'"):
...
if AutolevListener:
    class MyListener(AutolevListener):  # type: ignore
        ...
        def exitId(self, ctx):
            ...
            # A_C
            python_keywords = ["and", "as", "assert", "break", "class", "continue", "def", "del", "elif", "else",
                               "except", \
        ...
        def exitFunction(self, ctx):
            ...
            # Coef(y, x)
                ...
                if ch.expr(0) in self.matrix_expr and ch.expr(1) in self.matrix_expr:
                    ...
                    self.setValue(ctx, "_sm.Matrix([" + ", ".join(l) + "]).reshape(" + str(icount) + ", " + str(
                        jcount) + ")")
                else:
                    self.setValue(ctx, "(" + self.getValue(ch.expr(0)) +
            ...
            # Dot(Loop>, N1>)
                ...
                if ch.expr(1) in self.matrix_expr:
                    for i in range(num):
                        l.append("_me.dot(" + self.getValue(ch.expr(0)) + ", " + self.getValue(
                            ch.expr(1).getChild(0).expr(i)) + ")")
            ...
            # CM(P)
                ...
                if ch.getChildCount() == 4:
                    self.setValue(ctx,
                                  "_me.functions.center_of_mass(" + self.symbol_table2[ch.expr(0).getText().lower()] +
        ...
        def exitRegularAssign(self, ctx):
            ...
            if a in self.symbol_table.keys() and a in self.type.keys() and self.type[a] in ("variable",
                                                                                            "motionvariable"):
                    ...
                    if '_' in ctx.expr(0).getText().lower() and ctx.expr(0).getText().count('_') == 2:
                        ...
                        if vec[0] == "p":
                            self.write(v2 + ".set_pos(" + v1 + ", " + "(" + self.getValue(ctx.expr(0)) +
                                       ")" + ".express(" + self.symbol_table2[
                                           ctx.expr(1).getText().lower()] + f + "))\n")
                        elif vec[0] == "v":
                            ...
                            self.write(self.getValue(ctx.expr(0)) + " = " + "(" + self.getValue(
                                ctx.expr(0)) + ")" + ".express(" +
                                       self.symbol_table2[ctx.expr(1).getText().lower()] + f + ")\n")
                    else:
                        self.write(
                            self.getValue(ctx.expr(0)) + " = " + "(" + self.getValue(ctx.expr(0)) + ")" + ".express(" +
                            self.symbol_table2[ctx.expr(1).getText().lower()] + f + ")\n")
