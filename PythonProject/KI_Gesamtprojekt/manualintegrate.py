...
def proxy_rewriter(condition, rewrite):
    ...
    def _proxy_rewriter(criteria):
        ...
        integrand, symbol = integral
        debug("Integral: {} is rewritten with {} on symbol: {} and criteria: {}".format(integrand, rewrite, symbol,
                                                                                        criteria))
...
distribute_expand_rule = rewriter(
    lambda integrand, symbol: (
            isinstance(integrand, (Pow, Mul)) or all(
        arg.is_Pow or arg.is_polynomial(symbol) for arg in integrand.args)),
