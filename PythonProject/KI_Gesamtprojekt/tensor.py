...
class _TensorDataLazyEvaluator(CantSympify):
    ...
    def _get(self, key):
        ...
        if isinstance(key, TensMul):
            ...
            data_list = [self.data_from_tensor(i) if isinstance(i, Tensor) else i.data for i in tensmul_args if
                         isinstance(i, TensExpr)]
    ...
    def replace_with_arrays(self, replacement_dict, indices=None):
        ...
        for tensor, array in replacement_dict.items():
            ...
            if len(expected_shape) != array.rank() or (not all(dim1 == dim2 if
                                                               dim1.is_number else True for dim1, dim2 in
                                                               zip(expected_shape,
                                                                   array.shape))):
...
class TensMul(TensExpr, AssocOp):
    ...
    def contract_metric(self, g):
        ...
        dum = [(p0 - shifts[pos_map[p0]], p1 - shifts[pos_map[p1]]) for p0, p1 in dum if
               pos_map[p0] not in elim and pos_map[p1] not in elim]
...
class WildTensorHead(TensorHead):
    ...
    def __new__(cls, name, index_types=None, symmetry=None, comm=0, unordered_indices=False):
        ...
        obj = Basic.__new__(cls, name_symbol, Tuple(*index_types), sympify(symmetry), sympify(comm),
