...
from sympy.functions.special.gamma_functions import (digamma, gamma, loggamma, lowergamma, multigamma, polygamma,
                                                     trigamma, uppergamma)
...
def test_gamma_series():
    ...
    assert gamma(x).series(x, -1, 3) == \
           -1 / (x + 1) + EulerGamma - 1 + (x + 1) * (-1 - pi ** 2 / 12 - EulerGamma ** 2 / 2 + \
                                                      EulerGamma) + (x + 1) ** 2 * (
                       -1 - pi ** 2 / 12 - EulerGamma ** 2 / 2 + EulerGamma ** 3 / 6 - \
...
def test_lowergamma():
    ...
    assert lowergamma(70, 6) == factorial(
        69) - 69035724522603011058660187038367026272747334489677105069435923032634389419656200387949342530805432320 * exp(
        -6)
...
def test_uppergamma():
    ...
    assert uppergamma(70,
                      6) == 69035724522603011058660187038367026272747334489677105069435923032634389419656200387949342530805432320 * exp(
        -6)
...
def test_multigamma():
    ...
    assert conjugate(multigamma(x, p)).dummy_eq(pi ** ((conjugate(p) - 1) * \
                                                       conjugate(p) / 4) * Product(
        gamma(conjugate(x) + (1 - conjugate(_k)) / 2), (_k, 1, p)))
