...
def test_log():
    ...
    expr = tau ** 2 * ((tau - 1) * (tau + 1) * log(x + 1) / (tau ** 2 + 1) ** 2 + 1 / ((tau ** 2 \
                                                                                        + 1) * (x + 1)) - (
                                   -2 * tau * atan(x / tau) + (tau ** 2 / 2 - 1 / 2) * log(tau ** 2 \
                                                                                           + x ** 2)) / (
                                   tau ** 2 + 1) ** 2)
...
def test_issue_13416():
    assert limit((-x ** 3 * log(x) ** 3 + (x - 1) * (x + 1) ** 2 * log(x + 1) ** 3) / (x ** 2 * log(x) ** 3), x,
                 oo) == 1


def test_issue_13462():
    assert limit(n ** 2 * (2 * n * (-(1 - 1 / (2 * n)) ** x + 1) - x - (-x ** 2 / 4 + x / 4) / n), n,
                 oo) == x ** 3 / 24 - x ** 2 / 8 + x / 12
...
def test_issue_16708():
    ...
    assert limit((B * ti * vi - sqrt(m) * sqrt(-2 * B * d * vi + m * (vi) ** 2) + m * vi) / (B * vi), B, 0) == (
                d + ti * vi) / vi
