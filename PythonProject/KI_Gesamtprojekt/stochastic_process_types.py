...
class MarkovProcess(StochasticProcess):
    ...
    def probability(self, condition, given_condition=None, evaluate=True, **kwargs):
        ...
        if isinstance(condition, Relational):
            ...
            if len(rv) > 1:
                ...
                if isinstance(condition, (Eq, Ne)):
                    for i in range(0, n):
                        s += self.probability(Eq(rv[0], i), Eq(rv[1], i)) * self.probability(Eq(rv[1], i),
                                                                                             new_given_condition)
                    ...
                    for i in range(0, n):
                        if i <= n // 2:
                            for j in range(0, i + upper):
                                s += self.probability(Eq(rv[0], i), Eq(rv[1], j)) * self.probability(Eq(rv[1], j),
                                                                                                     new_given_condition)
                        else:
        ...
        if isinstance(condition, And):
            ...
            for i in range(1, len(sorted_ris)):
                ...
                mat_of = TransitionMatrixOf(self, mat) if isinstance(self, DiscreteMarkovChain) else GeneratorMatrixOf(
                    self, mat)
    ...
    def _symbolic_probability(self, condition, new_given_condition, rv, min_key_rv):
        ...
        if isinstance(condition, Relational):
            ...
            if isinstance(condition, (Eq, Ne)):
                ...
                total = Sum(self.probability(condition, new_given_condition),
    ...
    def expectation(self, expr, condition=None, evaluate=True, **kwargs):
        ...
        if isinstance(expr, Expr) and isinstance(condition, Eq) \
            ...
            mat_of = TransitionMatrixOf(self, mat) if isinstance(self, DiscreteMarkovChain) else GeneratorMatrixOf(self,
                                                                                                                   mat)
