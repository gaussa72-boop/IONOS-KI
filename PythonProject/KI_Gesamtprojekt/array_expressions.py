...
class ArrayTensorProduct(_CodegenArrayAbstract):
    ...
    def _canonicalize(self):
        ...
        if permutation_cycles:
            return _permute_dims(_array_tensor_product(*args),
        ...
        if contractions:
            ...
            contraction_indices = [tuple(cumulative_ranks[i] + k for k in j) for i, arg in contractions.items() for j in
                                   arg.contraction_indices]
        ...
        if diagonals:
            ...
            diagonal_indices = [tuple(cumulative_ranks2[i] + k for k in j) for i, arg in diagonals.items() for j in
                                arg.diagonal_indices]
...
class PermuteDims(_CodegenArrayAbstract):
    ...
    def _check_permutation_mapping(cls, expr, permutation):
        ...
        permutation_blocks = [[new_permutation[cumulative_subranks[i] + j] for j in range(e)] for i, e in
                              enumerate(subranks)]
        ...
        for i, cycle in enumerate(cyclic_form):
            ...
            for j in range(len(cumulative_subranks) - 1):
                if cyclic_min[i] >= cumulative_subranks[j] and cyclic_max[i] < cumulative_subranks[j + 1]:
                    ...
                    args[j] = _permute_dims(args[j],
                                            Permutation([[k - cumulative_subranks[j] for k in cyclic_form[i]]]))
                    flag = False
                    break
            if flag:
...
class ArrayContraction(_CodegenArrayAbstract):
    ...
    def __new__(cls, expr, *contraction_indices, **kwargs):
        ...
        free_indices_to_position = {i: i for i in range(sum(obj._subranks)) if
                                    all(i not in cind for cind in contraction_indices)}
    ...
    def _lower_contraction_to_addends(cls, expr, contraction_indices):
        ...
        contraction_indices_remaining = [Tuple.fromiter(j - shifts[j] for j in i) for i in
                                         contraction_indices_remaining]
    ...
    def _flatten(expr, *outer_contraction_indices):
        inner_contraction_indices = expr.contraction_indices
        outer_contraction_indices = ArrayContraction._convert_outer_indices_to_inner_indices(expr,
    ...
    def _ArrayContraction_denest_ArrayDiagonal(cls, expr: 'ArrayDiagonal', *contraction_indices):
        ...
        down_contraction_indices = expr._push_indices_down(expr.diagonal_indices, contraction_indices,
                                                           get_rank(expr.expr))
        # Flatten diagonally contracted indices:
        down_contraction_indices = [[k for j in i for k in (j if isinstance(j, (tuple, Tuple)) else [j])] for i in
    ...
    def _sort_fully_contracted_args(cls, expr, contraction_indices):
        ...
        contraction_indices_flat = {j for i in contraction_indices for j in i}
        fully_contracted = [all(j in contraction_indices_flat for j in range(cumul[i], cumul[i + 1])) for i, arg in
                            enumerate(expr.args)]
        new_pos = sorted(range(len(expr.args)),
                         key=lambda x: (0, default_sort_key(expr.args[x])) if fully_contracted[x] else (1,))
    ...
    def __init__(self, base_array: typing.Union[ArrayContraction, ArrayDiagonal, ArrayTensorProduct]):
        ...
        if isinstance(base_array, ArrayContraction):
            ...
            if isinstance(base_array.expr, ArrayContraction):
                ...
                diagonalized = ArrayContraction._push_indices_down(base_array.expr.contraction_indices,
                                                                   base_array.diagonal_indices)
